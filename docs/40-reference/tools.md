...
| `test_markdown.py` **shared** | the legal-Markdown renderer, tested the same way: exact outputs for the dialect, refusals for everything outside it, adversarial inputs that must degrade to visible literal text rather than markup or a hang (every case runs under a timeout — an input that never returns is a FAIL), and no tag outside the output vocabulary over the whole corpus |
