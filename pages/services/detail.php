<?php
/**
 * Tech4TIME — a service page that has no directory of its own.
 *
 * WHY THIS FILE EXISTS
 * Six services were built as directories, and each keeps its own index.php so
 * that every tool which walks the filesystem — audit_pages.py,
 * check_shared_markup.py, inject_icons.py — still finds it. A SEVENTH service,
 * added in the editor at admin.tech4time.bd, has no directory and never will:
 * nothing in this repository runs when content is published, and the editor
 * cannot create files here. So its address is routed to this file instead.
 *
 *     /pages/services/<slug>/  ->  /pages/services/detail.php?service=<slug>
 *
 * by the rewrite in .htaccess (section 3) and, locally, by tools/dev-router.php
 * — neither of which reads the other, so both carry the rule.
 *
 * The rewrite only fires when the request matches no real file or directory,
 * which is what keeps the six from reaching this file: their directories exist,
 * so they are served by their own index.php as before. This file is the route
 * for the ones that do not exist on disk.
 *
 * WHAT IT WILL NOT DO
 * The slug is matched against the document, never against the disk. It is not
 * joined to a path, and a value that is not a plain lowercase slug is refused
 * before anything else happens, so there is nothing here to traverse with.
 *
 * A slug the document does not have — or one whose service is hidden — answers
 * 404, the same as the six do. Reached at its own .php address, it redirects to
 * the clean URL, so the page is never reachable at two addresses.
 *
 * The body below is copied from the six, verbatim, and the bands are drawn by
 * lib/services.php exactly as they are there. Only the head's two absolute
 * URLs and the slug lookup differ. See ADR 0003 and ADR 0010.
 */
...
/**
 * Send a 404 the way the six directory pages do, and stop.
 */
function services_detail_missing(): never
{
...
$slug = is_string($_GET['service'] ?? null) ? $_GET['service'] : '';

/* The shape a slug is allowed to have, before it is used for anything. The
   editor mints slugs to this pattern; anything else cannot name a service. */
if ($slug === '' || !preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $slug) || strlen($slug) > 80) {
    services_detail_missing();
}
...
/* Reached at /pages/services/detail.php?service=... rather than through the
   rewrite, this page would answer on a second address, which is the
   duplicate-content problem .htaccess section 3 exists to prevent. Under the
   rewrite REQUEST_URI is still the clean URL, so this only fires on a direct
   hit. A slug that does have a directory lands there, which is correct: that
   directory's index.php is the page. */
if (str_contains($_SERVER['REQUEST_URI'] ?? '', 'detail.php')) {
    http_response_code(301);
    header('Location: /pages/services/' . rawurlencode($slug) . '/');
    exit;
}
?>
<!DOCTYPE html>
...
<link rel="canonical" href="https://tech4time.bd/pages/services/<?= h($slug) ?>/">
...
<meta property="og:url" content="https://tech4time.bd/pages/services/<?= h($slug) ?>/">
