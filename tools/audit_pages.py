...
def service_slugs() -> list[str]:
    """Every service a visitor can reach, read from the document.

    A service is a row in content/services.json, not a file, so the filesystem
    walk that finds every other page cannot find these. Without this, a seventh
    service is invisible to the audit: unchecked, and its link from the index
    reported as a page that has not been built.
    """
    document = ROOT / "content" / "services.json"
    ...
    rows = ((data.get("services") or {}).get("items")) or []
    return [row["slug"] for row in rows
            if isinstance(row, dict)
            and row.get("status") != "hidden"
            and isinstance(row.get("slug"), str)
            and re.fullmatch(r"[a-z0-9-]+", row["slug"])]
    ...
    if path.endswith("/") or target.is_dir():
        ...
        for name in ("index.html", "index.php"):
            if (target / name).is_file():
                return target / name
        # A service added in the editor has no directory; .htaccess rewrites
        # its clean URL to detail.php. Only slugs the document actually has
        # resolve this way, so a link to a service that does not exist is
        # still reported.
        match = re.fullmatch(r"/pages/services/([a-z0-9-]+)/", path)
        if match and match.group(1) in service_slugs() and DETAIL.is_file():
            return DETAIL
...
def render_php(path: Path, service: str | None = None) -> tuple[str, str | None]:
    ...
    if service is None:
        ...
        # A page that reads $_GET cannot be run with `php -f`, which has no
        # query string to give it. This hands it the one the rewrite would.
        assert re.fullmatch(r"[a-z0-9-]+", service), service
        argv = [php, "-r",
                f'$_GET["service"] = "{service}";'
                f' $_SERVER["REQUEST_URI"] = "/pages/services/{service}/";'
                f' require "{path}";']
...
def audit_page(path: Path, seen_titles: dict, seen_descriptions: dict,
               service: str | None = None) -> list[str]:
    rel = (f"/pages/services/{service}/  (rendered by {path.relative_to(ROOT)})"
           if service else path.relative_to(ROOT))
    problems = []

    if path.suffix == ".php":
        html, failure = render_php(path, service)
        if failure:
...
def check_admin_is_hidden() -> list[str]:
    ...
    # The sitemap is generated by sitemap.php and served at /sitemap.xml, so
    # what matters is what it SAYS, not what a file on disk says. Reading the
    # source would check the generator's comments and miss anything a document
    # put into the output.
    sitemap = ROOT / "sitemap.php"
    if sitemap.is_file():
        rendered, failure = render_php(sitemap)
        if failure:
            problems.append(f"sitemap.php: {failure}")
...
def main() -> None:
    ...
    # Every page that is a file, and then every service that is not. The second
    # list is what makes a service added in the editor audited rather than
    # merely reachable: same checks, same page, addressed the way the rewrite
    # addresses it.
    work: list[tuple[Path, str | None]] = [(f, None) for f in files if f != DETAIL]
    if DETAIL.is_file():
        work += [(DETAIL, slug) for slug in service_slugs()
                 if not (ROOT / "pages" / "services" / slug).is_dir()]

    print(f"Auditing {len(work)} page(s)\n")

    for path, service in work:
        problems = audit_page(path, seen_titles, seen_descriptions, service)
        real = [p for p in problems if "PENDING" not in p]
        soft = [p for p in problems if "PENDING" in p]

        label = (f"/pages/services/{service}/  (no directory; via detail.php)"
                 if service else path.relative_to(ROOT))
